...
#define DEBUG_INFO(fmt, arg...)		do { if (dmsg&(1<<0)) printk(KERN_INFO fmt,##arg); } while(0)
#define DEBUG_NOTICE(fmt, arg...)	do { if (dmsg&(1<<1)) printk(KERN_INFO fmt,##arg); } while(0)
#define DEBUG_DBG(fmt, arg...)		do { if (dmsg&(1<<2)) printk(KERN_INFO fmt,##arg); } while(0)
#define DEBUG_DUMP(fmt, arg...)		do { if (dmsg&(1<<3)) printk(KERN_INFO fmt,##arg); } while(0)
					...
					struct usb_ctrlrequest *crq, u32 ep0csr)
{
	...
	************************************/
	...
	DEBUG_DBG("ep0state *** %s\n", ep0states[dev->ep0state]);
	DEBUG_DBG("<<< sp_udc_handle_ep0_idle...\n");
}

static void sp_udc_handle_ep0_proc(struct sp_ep *ep, struct sp_request *req)
{
	struct usb_ctrlrequest crq;
	u32 ep0csr;
	...
	case EP0_IDLE:
		...
		sp_udc_handle_ep0_idle(dev, ep, &crq, ep0csr);
		break;
	...
	case EP0_OUT_DATA_PHASE:
		...
		if(sp_udc_read_ep0_fifo(ep, req)){
			ep->dev->ep0state = EP0_IDLE;
			DEBUG_INFO("****ep0 out1 done\n");
		}
		break;

	case EP0_END_XFER:
		DEBUG_DBG("EP0_END_XFER ... what now?\n");
		dev->ep0state = EP0_IDLE;
		break;

	case EP0_STALL:
		DEBUG_DBG("EP0_STALL ... what now?\n");
		dev->ep0state = EP0_IDLE;
		break;
	}
}
				    ...
				    struct sp_request *req)
{
	...
	else udc_write(udc_read(UDLIE) | EP0O_IF, UDLIE);
	...
	DEBUG_INFO("read ep0: count=%d, actual=%d, length=%d, last=%d, zero=%d\n", count, req->req.actual, req->req.length, is_last, req->req.zero);
...
static int sp_udc_bulkout_pio(u8 *buf, u32 avail)
{
				       ...
				       struct sp_request *req)
{
	...
	DEBUG_INFO(">>> %s UDEPBFS = %xh\n", __FUNCTION__, udc_read(UDEPBFS));

	DEBUG_INFO("1.req.length=%d req.actual=%d req->req.dma = %xh UDEPBFS = %xh \n", req->req.length, req->req.actual, req->req.dma, udc_read(UDEPBFS));
	...
	DEBUG_INFO("3.req.length=%d req.actual=%d UDEPBFS = %xh count=%d is_last=%d\n", req->req.length, req->req.actual, udc_read(UDEPBFS), count, is_last);
...
static int sp_udc_bulkin_pio(struct sp_ep *ep, struct sp_request *req)
{
	...
	DEBUG_INFO("1.req.actual = %d req.length=%d req->req.dma=%xh UDEP12FS = %xh\n", req->req.actual, req->req.length, req->req.dma, udc_read(UDEP12FS));
	...
	DEBUG_INFO("3.req.actual = %d, count = %d, req.length=%d, UDEP12FS = %xh, is_last = %d\n", req->req.actual, count, req->req.length, udc_read(UDEP12FS), is_last);
...
static irqreturn_t sp_udc_irq(int irq, void *_dev)
{
	...
	if (irq_en2_flags & SUS_IF) {
		DEBUG_NOTICE(">>> IRQ: Suspend \n");
		DEBUG_NOTICE("clear  Suspend Event\n");
	...
	// if (irq_en2_flags & (EP0S_IF|EP0I_IF|EP0O_IF)) {
...
// static int sp_read_reg(struct usb_ep *_ep, struct usb_request *_req,
// 			    gfp_t gfp_flags)
// {
// }
			    ...
			    gfp_t gfp_flags)
{
	...
	if (list_empty(&ep->queue) ) {
		if (idx == EP0) {
			...
			if (sp_udc_handle_ep(ep, req)) req = NULL;
		}
	}
	else
		sp_sendto_workqueue(dev, idx);

	DEBUG_DBG("req = %x, ep=%d \n", req, idx);
	if (likely(req)) {
...
static int sp_init_work(int ep_num, char *desc, void (*func)(struct work_struct*))
{
	...
	if (!udc->wq[ep_num].queue) {
		DEBUG_ERR("cannot create workqueue %s\n", desc);
		ret = -ENOMEM;
	}
...
void usb_switch(int device)
{
	...
	if (device) {
		...
		DEBUG_DBG("host to device\n");
	} else {
		udc_write(udc_read(UDLCSET) | SOFT_DISC, UDLCSET);
		writel(RF_MASK_V_SET(1 << 5), regs + USBC_CTL_OFFSET);
		DEBUG_DBG("device to host!\n");
	}
}
